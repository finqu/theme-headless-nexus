 ...
 *
 * Uses the SDK `GET_PRODUCT_GROUP` document with cache presets — the
 * `getProductGroup` helper does not accept Next.js fetch options.
 */
...
/**
 * Fetch a category listing page. `GET_PRODUCT_GROUP_WITH_PRODUCTS` looks up by
 * handle, so we resolve handle from the resource ID first.
 */
