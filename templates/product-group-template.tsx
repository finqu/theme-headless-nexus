...
export async function ProductGroupTemplate({
  ...
  try {
    ...
    let listing = await getProductGroupListing({
      id,
      locale,
    ...
    for (let current = 1; current < walkTo; current += 1) {
      const nextCursor = listing.products.pageInfo.endCursor;
      if (!listing.products.pageInfo.hasNextPage || !nextCursor) {
        break;
      }
      cursor = nextCursor;
    ...
    const products = listing.products.nodes ?? [];
    const totalCount = listing.products.totalCount ?? listing.productsCount ?? products.length;
    const totalPages = Math.max(1, Math.ceil(totalCount / PRODUCTS_PER_PAGE));
    const children = (listing.productGroups ?? []).filter(
      (child): child is ProductGroup & { title: string } => Boolean(child?.title)
    );
    const ancestors = (listing.breadcrumbs ?? []).filter(
      (crumb) => crumb?.title && crumb.id !== listing.id
    );
